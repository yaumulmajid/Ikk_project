package id.go.lan.ikk.module.dashboard.model;

import id.go.lan.ikk.module.policy.model.AgendaSettingDto;
import id.go.lan.ikk.module.policy.model.EvaluasiKebijakanDto;
import id.go.lan.ikk.module.policy.model.FormulasiKebijakanDto;
import id.go.lan.ikk.module.policy.model.ImplementasiKebijakanDto;
import lombok.AllArgsConstructor;
import lombok.Getter;
import lombok.NoArgsConstructor;
import lombok.Setter;

@AllArgsConstructor
@NoArgsConstructor
@Getter
@Setter
public class DashboardPolicyProgressDto {
    private AgendaSettingDto agendaSetting;
    private FormulasiKebijakanDto formulasiKebijakan;
    private ImplementasiKebijakanDto implementasiKebijakan;
    private EvaluasiKebijakanDto evaluasiKebijakan;
}
